...
def _is_auth_disabled() -> bool:
    """Return True when authentication is explicitly disabled for local development."""
    return os.getenv("DISABLE_AUTH", "false").lower() in ("1", "true", "yes", "on")


def _get_dev_user_info() -> Dict[str, Any]:
    """Synthetic user context used when auth is disabled."""
    return {
        "id": "dev-user",
        "email": "dev@queryweaver.local",
        "name": "Dev User",
        "picture": None,
        "provider": "dev",
    }
...
async def validate_user(request: Request) -> Tuple[Optional[Dict[str, Any]], bool]:
    ...
    try:
        if _is_auth_disabled():
            return _get_dev_user_info(), True
